...
use std::fs;
use std::path::Path;
use crate::analyzer::{ScanSummary, RuleViolation};
...
fn format_table(summary: &ScanSummary) -> Result<String> {
    ...
    cf_table.add_row(Row::new(vec![Cell::new("Cyclomatic Complexity"), Cell::new(&summary.cyclomatic_complexity.to_string())]));
    cf_table.add_row(Row::new(vec![Cell::new("Conditional Branches"), Cell::new(&summary.conditional_branch_count.to_string())]));
    cf_table.add_row(Row::new(vec![Cell::new("Path Count (estimate)"), Cell::new(&summary.path_count_estimate.to_string())]));
    cf_table.add_row(Row::new(vec![Cell::new("Path Count (exact, capped)"), Cell::new(&summary.path_count_exact.map(|v| v.to_string()).unwrap_or_else(|| "n/a".to_string()))]));
    cf_table.add_row(Row::new(vec![Cell::new("Max Stack Depth (bytes)"), Cell::new(&summary.max_stack_depth.to_string())]));
    cf_table.add_row(Row::new(vec![Cell::new("CFG Entry Offset"), Cell::new(&summary.cfg_entry_offset.to_string())]));
    cf_table.add_row(Row::new(vec![Cell::new("CFG Exit Offsets"), Cell::new(&format!("{:?}", summary.cfg_exit_offsets))]));
    cf_table.add_row(Row::new(vec![Cell::new("Unreachable Blocks"), Cell::new(&format!("{:?}", summary.cfg_unreachable_blocks))]));
    cf_table.add_row(Row::new(vec![Cell::new("CFG Max Depth"), Cell::new(&summary.cfg_max_depth.to_string())]));
    cf_table.add_row(Row::new(vec![Cell::new("CFG Avg Out-Degree"), Cell::new(&format!("{:.2}", summary.cfg_avg_out_degree))]));
    cf_table.add_row(Row::new(vec![Cell::new("CFG Max Out-Degree"), Cell::new(&summary.cfg_max_out_degree.to_string())]));
...
pub fn generate_report(summary: &ScanSummary, output_path: &Path) -> Result<()> {
    ...
    if summary.violations.is_empty() {
        ...
        for violation in &summary.violations {
            report.push_str(&format!("### {} - {}\n\n", violation.rule_id, format_severity_md(&violation.severity)));
