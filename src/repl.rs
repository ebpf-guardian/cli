        ...
        match line.trim() {
            ...
            ":summary" => {
                if let Some(s) = &state {
                    println!(
                        "{}",
                        crate::output::formatter::format_output(
                            s,
            ...
            ":cfg" => {
                if let Some(s) = &state {
                    if let Some(ascii) = &s.cfg_ascii {
                        println!("{ascii}");
                    } else {
                        println!("No CFG ASCII available.");
                    }
                } else {
                    println!("No analysis yet.");
                }
            ...
            ":unreachable" => {
                if let Some(s) = &state {
                    println!("Unreachable blocks: {:?}", s.cfg_unreachable_blocks);
                } else {
                    println!("No analysis yet.");
                }
            }
            ":metrics" => {
                if let Some(s) = &state {
                    ...
                    println!(
                        "Out-degree avg/max: {}/{}",
                        s.cfg_avg_out_degree, s.cfg_max_out_degree
                    );
                } else {
                    println!("No analysis yet.");
                }
